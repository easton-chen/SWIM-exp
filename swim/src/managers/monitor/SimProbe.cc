 ...
 * @czy
 */
double SimProbe::getBasicMedianResponseTime() {
    return basicResponseTime.getMedian();
}

double SimProbe::getOptMedianResponseTime() {
    return optResponseTime.getMedian();
}

double SimProbe::getTimeoutRate() {
    double rate = 1.0 * (basicResponseTime.getTimeoutNum() + optResponseTime.getTimeoutNum()) / (optResponseTime.getCount() + basicResponseTime.getCount());
    return rate;
}
...
Observations SimProbe::getUpdatedObservations() {
     ...
     * @czy
     */
    obs.basicMedianResponseTime = getBasicMedianResponseTime();
    obs.optMedianResponseTime = getOptMedianResponseTime();
    obs.timeoutRate = getTimeoutRate();
